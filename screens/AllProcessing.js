...
const API_URL = "http://3.141.31.155:80/";
// const API_URL = "http://10.0.0.2:8000/";
          ...
          {selectedImages.map((image, index) => {
            ...
            return (
              <View key={index} style={styles.imageContainer}>
                ...
                {matchedResults.map((result, resultIndex) =>
                  result.sub_model_results?.map((subResult, subIndex) => (
                    <View key={`${resultIndex}-${subIndex}`} style={styles.classLabelContainer}>
                      <Text style={styles.classLabelText}>
                        {subResult.detected_classes?.join(", ") || "Unknown"}
                      </Text>
                    </View>
                  ))
                )}
...
const styles = StyleSheet.create({
  container: { position: "sticky",flex: 1, backgroundColor: "#fff", alignItems: "center", justifyContent: "center", padding: 20},
